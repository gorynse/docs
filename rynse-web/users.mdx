...
The **Users** page is where you control who can log in to your Rynse portal at `app.v2.gorynse.com`. Unlike vehicles, you manage user access yourself — add a new teammate, reset a password, or remove access in a couple of clicks.
...
<Warning>
  We actively recommend against giving drivers portal access. The portal is built for the people running the program, not the people using it day-to-day at the wash. Keep the user list short and intentional — it makes audits, password resets, and offboarding much easier.
</Warning>
...
| **User Type** | The role assigned to the user. Today this is always **Fleet Manager**. |

<Note>
  **One role today.** Every portal user is a Fleet Manager — there are no per-tab permission tiers. Anyone you add can see everything in the portal. If we ever ship role-based permissions, this page will be updated.
</Note>
...
<Steps>
  ...
  <Step title="Fill in the form">
    ...
    | **User Type** | Defaults to **Fleet Manager** — the only option today. |
  </Step>
...
The new user gets an email from `support@gorynse.com` with a temporary password. On their first login at `app.v2.gorynse.com`, they're prompted to change it to something only they know. From there, they have the same access you do.
...
If a teammate can't log in, isn't receiving the temp password email, or you'd like a hand cleaning up old accounts, email `support@gorynse.com` and we'll sort it out. If invoice or credential emails aren't landing, ask your IT team to whitelist `@gorynse.com` — corporate firewalls are the most common reason new users miss the welcome email.
