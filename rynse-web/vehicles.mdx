...
<Info>
  **Most changes go through Rynse.** Adding or archiving a vehicle, swapping a card between vehicles, ordering new cards, and changing memberships are all handled by your CS team — see [Asking us to make changes](#asking-us-to-make-changes) below. **A few fields are editable inline by double-clicking the cell:** Description, VIN, and License Plate. Card status actions (Freeze, Close, Change Facility) are covered further down.
</Info>

<Tip>
  Tables default to showing 10 rows. Bump the page size to 50 from the dropdown at the bottom of the table — you'll usually want to see your whole fleet at once.
</Tip>
...
| **Card** | Last 4 of the Rynse card currently attached. Icons in this column tell you the card's state at a glance: **green check** = active; **snowflake** = frozen (paused, reversible); **red X** = closed (permanent); **red card** = no card attached. |
...
<Note>
  **The padlock icon means merchant rules are configured — not that the card is frozen.** Frozen cards show the snowflake icon in the **Card** column. See [Spend Controls](/rynse-web/spend-controls) for what each rule type does.
</Note>
...
Every wash, fuel stop, and visit is attached to the **vehicle**, not the card that paid for it. So when a card is closed and reissued, the new card inherits the same vehicle history — open any vehicle row and the clock icon shows the full timeline regardless of which physical card was used.

This is why "I closed the card, where did the history go?" is never a real problem in Rynse.
...
**Quick read on the two card actions:** _freezing_ is reversible — like asking your credit-card company to pause a card. _Closing_ is permanent — a new card has to be issued. Use freeze for misplaced or temporarily out-of-service vehicles; reserve close for cards that are confirmed lost, stolen, or compromised.
...
<AccordionGroup>
  ...
  <Accordion title="The last 4 of the card doesn't match my asset number — how do I match a card to a vehicle?">
  ...
  <Accordion title="Can I get driver-level attribution (e.g., who washed using this vehicle's card)?">
    Not today. Rynse attributes every transaction to the vehicle, not the driver. If you need to control timing or limit usage, ask your CSM about schedule and per-transaction caps in [Spend Controls](/rynse-web/spend-controls).
  </Accordion>
  <Accordion title="Can I order more cards from the portal?">
    Not today. New cards are ordered by your CSM — email `support@gorynse.com` with the vehicles that need cards and we'll handle production and shipping.
  </Accordion>
