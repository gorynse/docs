...
```mermaid
flowchart LR
    A["<b>1 · Submit request</b><br/>facility · vehicles<br/>services · date window"] --> B["<b>2 · Scheduled</b><br/>vendor matched<br/>date confirmed"]
    B --> C["<b>3 · On the schedule</b><br/>visit visible on<br/>the calendar"]
    C --> D(("<b>4 · Wash on-site</b><br/><i>happens offline</i>"))
    D --> E["<b>5 · Work order complete</b><br/>invoice ready<br/>in the portal"]

    classDef stage fill:#EEEBFF,stroke:#5A47FF,stroke-width:1.5px,color:#1F1A4D
    classDef offline fill:#F4F6FA,stroke:#CDD3DD,stroke-width:1.5px,color:#475063,stroke-dasharray: 4 3
    class A,B,C,E stage
    class D offline
```
...
| --- | --- | --- |
| **1 · Submit request** | Open **Mobile Wash → Work Orders** and create a request: facility, vehicles, services, date window, notes. | _"Work order requested"_ confirmation. |
| **2 · Scheduled** | Nothing — Rynse coordinates with the vendor on your behalf. | _"Your work order is scheduled for [date]"_ notification once a vendor and date are locked in. |
| **3 · On the schedule** | Optional — check the Schedule view to confirm timing or share with a colleague. | The visit on the calendar, plus reminder + reschedule updates as the date approaches. |
| **4 · Wash on-site** | Nothing in the portal. Vendor's crew shows up and washes the listed vehicles. | _(offline — no portal activity)_ |
| **5 · Work order complete** | Pull the invoice from the work order; reconcile against the monthly invoice cycle. | _"Work order complete — invoice attached"_ notification with the per-WO invoice. |
