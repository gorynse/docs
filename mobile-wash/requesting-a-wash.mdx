...
<Steps>
  <Step title="Step 1 — Location & vehicles">
    ...
    <Note>
      The table lists your whole organization's fleet, not only the chosen facility's vehicles. Vehicles at the selected facility come first, then the rest of your fleet, and the **Facility** column shows where each one is assigned — so you can still find and add a vehicle that's filed under another location. Archived vehicles are left out — only active vehicles appear, so you can't request a wash for one you've retired.
    </Note>
  </Step>
